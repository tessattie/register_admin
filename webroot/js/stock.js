	...
	$('#product_search').on('select2:select', function (e) {
		...
		$.ajax({
	       ...
	       success : function(data, statut){
	       	...
	       	var append = '<tr class="product_element" id="'+data.id+'"><td class="remove_product text-center"><input name="product_id[]" type="hidden" value="'+data.id+'"><i class="feather icon-trash"></i></td><td class="product_name">' + data.barcode + ' - ' + data.name + '</td><td class="product_quantity"><input name="quantity[]" value="1" class="form-control edit_product"></td> <td class="product_cost"><input name="cost[]" value="'+cost+'" class="form-control edit_product"></td><td class="text-right product_total"><span class="total">'+cost+'</span></td></tr>';
	       	var exists = $("#"+data.id); 
	       	if(exists.attr('class') == 'product_element'){
	       		// augment quantity
	       		var quantity = parseFloat(exists.find(".product_quantity input").val()) + 1;
	       		exists.find(".product_quantity input").val(quantity);
	       		update_costs();
