    ...
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <th scope="col"><?= $this->Paginator->sort('requisition_number') ?></th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?= $order->has('supplier') ? $this->Html->link($order->supplier->name, ['controller' => 'Suppliers', 'action' => 'view', $order->supplier->id]) : '' ?></td>
                <td><?= h($order->created) ?></td>
                <td><?= h($order->modified) ?></td>
                <td><?= $order->has('user') ? $this->Html->link($order->user->id, ['controller' => 'Users', 'action' => 'view', $order->user->id]) : '' ?></td>
                <td><?= $this->Number->format($order->status) ?></td>
                <td><?= $this->Number->format($order->type) ?></td>
                <td><?= $this->Number->format($order->fees) ?></td>
                <td><?= $this->Number->format($order->rate_id) ?></td>
                <td><?= $this->Number->format($order->purchase_rate) ?></td>
                <td><?= h($order->requisition_number) ?></td>
